...
**あまり入っていない。** 調査時点で検証していた画像・動画モデル 47 件のうち、持っていたのは 7 件。
...
判定にはすべて根拠が付き、ルールごとに裏付けの強さも表示される。実ファイルで確認したものは無印、フォーマットを書き出す実装のソースから取ったものは `[導出・実測ではない]`、推測のものは `[未検証・推定]`。現時点で推測はゼロ（実測 50 / 導出 6）だが、新しいアーキテクチャの登場には追いつかなくなる。
...
ルールは記憶からでなく実ファイルで検証している。`tools/verify_rules.py` は公開モデル 53 件のヘッダだけを HTTP Range で取得し（重みはダウンロードせず、再配布もしない）、期待する判定結果と照合する。
...
うち 5 件は gated リポジトリ（FLUX.1、SD3.5）。アカウントなしで実行するとその 5 件は飛ばされ、残り 48 件は通る。含めるには Hugging Face で自分でライセンスに同意し、`HF_TOKEN` を設定する。
