...
このファイルが何から作られたか。メタデータに記録が残っている場合だけ出る。
根拠として、どこから読んだかも併記する。

```
  元モデル    a2.safetensors + animayume_v05.safetensors
              根拠: ComfyUI 実行グラフ
              設定: ModelMergeSimple.ratio=0.5
```

記録の経路は 3 つあり、どれか 1 つが残っていれば辿れる。

| 経路 | 何が分かるか |
| --- | --- |
| `modelspec.merged_from` | マージ元の名前。比率は残らない |
| `merged_loras` + `merged_strengths` | 親 LoRA と各々の強度。`名前 (0.3)` の形で対応付けて出す |
| ComfyUI の `prompt` / `workflow` | 読み込んだモデル名と、ノードごとの数値設定 |

強度は、名前と個数が一致するときだけ対応付ける。合わないときは名前だけ出す。
**間違った強度を出すより、出さない方がましだから。**

ComfyUI のグラフからは、ノード種別の一覧を持たずに「モデルファイルらしい値」を拾っている。
`.safetensors` は実測、他の拡張子は導出。数値は解釈せず、どのノードのどの入力にあったかを
そのまま `設定` に出す（`ratio` 以外は実物を見ていないため）。

手元で確認した限り、親を記録しているファイルは 38 件中 3 件だけだった。
...
親を記録している 3 つの経路（`マージ元`、`マージした LoRA` + `マージ強度`、
ComfyUI のグラフ）は、**`元モデル` 行にまとめて出る**。次の節を参照。
...
クリックすると原寸で開く。220px に収めた表示では潰れている細部が見える。

端末には画像を出せないので、代わりに何であるかを出す。

```
  見本画像: JPEG 画像、9.8 KB
```

画像そのものが要るときは `--thumbnails DIR`。見つけたものを全部ファイルとして
書き出し、走査した構造を再現する。GUI にも同じものがチェックボックスであり、
そちらはレポートの隣のフォルダに置く。

宣言された種別は信用しない。何であるかは先頭のバイトで決めるので、
`image/jpeg` と書かれた PNG は `.png` として書き出す。
画像でないものは書き出さず、宣言されていた種別つきで報告する。
...
持っているファイルは少ない。このプロジェクトが検証に使っている公開モデル 47 件のうち 7 件で、
いずれも Stability AI か Black Forest Labs のチェックポイントかバックボーン。
実測した動画モデルは 1 件も持たず、VAE・Text Encoder・ControlNet・Embedding も同様。
