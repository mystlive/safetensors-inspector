...
LABELS = {
    "en": {
        ...
        "meta_image": "{kind} image, {size}",
        "meta_image_broken": "{reason}",
        "img_err_not_uri": "declared as an image, but the value is not a data URI",
        "img_err_not_image": "not an image: {mime}",
        "img_err_decode": "declared {mime}, but the base64 does not decode",
        "img_err_mismatch": "declared {mime}, but the bytes are not a format we recognise",
        ...
        "wrote_thumbnails": "Wrote {n} embedded image(s) to: {path}",
        "thumbnail_failed": "could not extract from {name}: {reason}",
        ...
        "gui_output_default_path": "leave empty to write {path}",
        "gui_browse": "Browse...",
        ...
        "help_html": "write a self-contained HTML report: a sortable, searchable table whose rows open into the full report. Pass 'auto' instead of a path to name it after the folder scanned",
        ...
        "help_thumbnails": "write embedded preview images out to a directory, mirroring the scanned tree",
    ...
    "ja": {
        ...
        "meta_image": "{kind} 画像、{size}",
        "meta_image_broken": "{reason}",
        "img_err_not_uri": "画像と宣言されているが、値が data URI の形をしていない",
        "img_err_not_image": "画像ではない: {mime}",
        "img_err_decode": "{mime} と宣言されているが、base64 をデコードできない",
        "img_err_mismatch": "{mime} と宣言されているが、中身が知っている画像形式ではない",
        ...
        "wrote_thumbnails": "埋め込まれていた画像 {n} 件を書き出した: {path}",
        "thumbnail_failed": "{name} から取り出せなかった: {reason}",
        ...
        "gui_output_default_path": "空欄なら {path} に書く",
        "gui_browse": "参照...",
        ...
        "help_html": "自己完結の HTML レポートを書き出す（並べ替え・検索できる一覧。行を開くと詳細）。パスの代わりに 'auto' を渡すと、走査したフォルダ名から名前を作る",
        "help_out": "結果をファイルに書き出す（UTF-8 BOM 付き。メモ帳や Excel でそのまま開ける）",
        "help_unresolved": "判別できなかったファイルを、ルール追加に必要な情報つきで書き出す",
        "help_thumbnails": "埋め込まれた見本画像をフォルダに書き出す。走査した構造を再現する",
